...
2. Verify Node 22 and 24 CI: build, typecheck, lint/security scan, tests,
   isolation, fixtures, dependency licenses, packing, and coverage.
...
8. Publish `@glyphkiln/core` manually with npm provenance after inspecting
   `npm pack --dry-run`.
9. Verify installation, package exports, CLI, and direct/isolated SVG/PNG
   renders from the published tarball.
