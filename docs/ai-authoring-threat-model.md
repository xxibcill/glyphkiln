...
7. Later App workflow code must independently enforce stored locks, resolve
   exact workspace-owned resources, revalidate the final document, and record
   the human decision.
...
| Selective regeneration changes a human lock                              | No lock/vary operation exists in this slice                                                    | Define server-owned lock semantics and compare against the stored base independently of model claims |
...
- server-owned lock definitions for copy, image, crop, typography, palette,
  and composition, including overlap rules;
