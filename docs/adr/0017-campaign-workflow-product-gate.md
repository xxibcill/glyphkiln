...
handoff bundle. A reviewed promotion changes both the evidence record and the
runtime status, preventing an environment variable from self-certifying the
gate.

## Consequences

- Existing installations remain on the manual design/review workflow unless an
  operator makes the explicit qualification assertion after checked-in PASS.
